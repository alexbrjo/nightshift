...
export default function JobsPage() {
    ...
    async function fetchJobs() {
        try {
            const response = await axios.get(`http://localhost:8000/api/jobs?project_id=${projectId}`);
    ...
    return (<div className="jobs-page">
      ...
      {jobs.length === 0 ? (<div className="empty-state">
          ...
          {jobs.map((job) => {
                ...
                <p>{job.description}</p>
                {job.status !== 'running' && (<button className="btn-secondary" onClick={() => handleRunJob(job.id)}>
                    ▶ Run Job
                  </button>)}
              </div>);
            })}
