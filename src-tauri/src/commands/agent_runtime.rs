...
pub(super) fn method_agent_instructions() -> &'static str {
    concat!(
        ...
        "Use the provided Method helper tools only to create a new Method skeleton or explain a Method file after edits. ",
        "Nightshift owns durable Method draft state; do not pretend a Method is executable while blockers remain. ",
        "Edit Method YAML as a normal project file with the filesystem tools available in this conversation, using project-relative file paths the user gives you; ",
        "do not claim to inspect files unless a tool result provided their contents. ",
        "Represent prompt, data, JSON schema, script, and api_key inputs as type: resource workflow nodes. ",
        "Put shared model names, provider settings, temperature, token limits, default sample counts, default strategy, and model sweep values in Method provider/parameters. ",
        ...
        "Prefer creating a concise draft with a DAG of resource, sample, inference, transform, and analysis nodes when details are not yet known. ",
        ...
        "script resource nodes feed transform nodes. ",
